...
--[[--------------------------------------------------------------------------------------------
TBC Compatibility Functions
--]]--------------------------------------------------------------------------------------------

-- Safe SetText for CheckButton labels
function Repondeur_SetButtonText(buttonName, text)
	local fontstring = getglobal(buttonName .. "Text")
	if fontstring then
		fontstring:SetText(text)
	end
	-- In TBC, if fontstring doesn't exist, we simply skip it
	-- The label will be created via XML or will be missing (non-critical)
end
...
function Repondeur_InitializeSetup()
	...
	if VariablesLoaded and RepondeurMainOptions[Realm][Player]["ActiveMod"] then
		...
		RepondeurFrame:SetScript("OnEvent", function(__, event, arg1, arg2, arg3, arg4)
			...
			if status == "GM" then
				DEFAULT_CHAT_FRAME:AddMessage("::: Repondeur ::: GM MESSAGE DETECTED, NOT REPLYING");
			else
