...
-- Mixin compatibility (doesn't exist in TBC)
local function Mixin(dst, ...)
	if type(dst) ~= "table" then
		error("Mixin: First argument must be a table")
	end
	for i = 1, select("#", ...) do
		local src = select(i, ...)
		if src then
...
local function GetAddOnMetadataSafe(addonName, field)
	if GetAddOnMetadata then
		...
		-- TBC fallback: return a default version string
		return "3.04.1"
	end
end
...
function AutoFlood_OnEvent(self, event)
	...
	if event == "VARIABLES_LOADED" then
		...
		-- Add-on version
		local version = GetAddOnMetadataSafe("SelfRecruitment", "Version")
...
_G["Flood_Config"] = AF_characterConfig
...
SLASH_AUTOFLOODSETRATE1 = "/floodrate" --fixe la fréquence en secondes du Flood ex: 120
